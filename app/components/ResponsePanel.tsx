...
export default function ResponsePanel() {
  ...
  const handleCopy = () => {
    if (request.response?.data) {
      navigator.clipboard.writeText(request.response.data);
      setCopied(true);
          ...
          <div className="mb-4">
            <div className="flex items-center">
              <span className={`px-3 py-1 rounded-lg text-sm ${
                request.response.status && request.response.status >= 200 && request.response.status < 300 ? 'bg-green-100 text-green-800' :
                  request.response.status && request.response.status >= 400 && request.response.status < 500 ? 'bg-yellow-100 text-yellow-800' :
                    'bg-red-100 text-red-800'
              }`}>
                {request.response.status} {request.response.status === 200 ? 'OK' : ''}
              </span>
              <span className="ml-2 text-sm text-gray-600">
                耗时: {request.response.time}ms
              </span>
              <span className="ml-2 text-sm text-gray-600">
